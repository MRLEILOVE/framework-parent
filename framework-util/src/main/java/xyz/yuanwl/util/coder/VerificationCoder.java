...
/**
 * 验证码工具，包括字符串验证码（短信验证码）和图片验证码
 * <ul>
 * <li>可生成数字、大写、小写字母及三者混合类型的验证码</li>
 * <li>支持自定义验证码字符数量,支持自定义验证码图片的大小,支持自定义需排除的特殊字符,支持自定义干扰线的数量,支持自定义验证码图文颜色</li>
 * <li>另外,给Shiro加入验证码有多种方式,也可以通过继承修改FormAuthenticationFilter类,通过Shiro去验证验证码</li>
 * <li>而这里既然使用了SpringMVC,也为了简化操作,就使用此工具生成验证码,并在Controller中处理验证码的校验</li>
 * </ul>
...
public class VerificationCoder {
	...
	/** 默认要排除的字符 */
	private static final String DEFAULT_EXCLUDE_STRING = "1,i,I,l,L,0,o,O,2,z,Z,9,g,G";
	...
	public static String generateTextCode(int type, int length, String excludeString) {
		...
		if (StringUtils.isBlank(excludeString)) {
			excludeString = DEFAULT_EXCLUDE_STRING;
		}
		...
		case TYPE_LETTER_ONLY:
			while (i < length) {
				...
				if ((t >= 97 || (t >= 65 && t <= 90)) && excludeString.indexOf((char) t) < 0) {
		...
		case TYPE_ALL_MIXED:
			while (i < length) {
				int t = random.nextInt(123);
				if ((t >= 97 || (t >= 65 && t <= 90) || (t >= 48 && t <= 57)) && excludeString.indexOf((char) t) < 0) {
					verifyCode.append((char) t);
					i++;
				}
			}
		...
		case TYPE_NUM_UPPER:
			while (i < length) {
				...
				if ((t >= 65 || (t >= 48 && t <= 57)) && excludeString.indexOf((char) t) < 0) {
					verifyCode.append((char) t);
					i++;
				}
			}
		...
		case TYPE_NUM_LOWER:
			while (i < length) {
				int t = random.nextInt(123);
				if ((t >= 97 || (t >= 48 && t <= 57)) && excludeString.indexOf((char) t) < 0) {
					verifyCode.append((char) t);
					i++;
				}
			}
		...
		case TYPE_LOWER_ONLY:
			while (i < length) {
				int t = random.nextInt(123);
				if ((t >= 97) && excludeString.indexOf((char) t) < 0) {
					verifyCode.append((char) t);
					i++;
				}
			}
	...
	public static BufferedImage generateImageCode(String textCode, int width, int height, int interLine,
			boolean randomLocation, Color bgColor, Color fontColor, Color lineColor) {
		// 创建内存图像
		BufferedImage bufferedImage = new BufferedImage(width, height, BufferedImage.TYPE_INT_RGB);
		// 获取图形上下文
		Graphics graphics = bufferedImage.getGraphics();
		// 画背景图
		graphics.setColor(null == bgColor ? generateRandomColor() : bgColor);
		...
		// 字符横轴之间的间隔
		int interval = (width - fsize * textCode.length()) / (textCode.length() + 1);
		interval = interval > 0 ? interval : 0; // 确保间隔大于等于0
		// 横轴坐标
		int fx = (int) (interval + fsize * 0.2);
		// 纵轴坐标
		int fy = fsize;
		// 设定字体
	...
	public static BufferedImage generateImageCode(String textCode, int width, int height) {
		return generateImageCode(textCode, width, height, 5, true, Color.WHITE, Color.BLACK, null);
	}
}